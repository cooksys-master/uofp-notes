...
public class OrgChart {
	...
	public boolean addEmployee(Employee employee) {
		...
		// An employee to be added should have their chain of command represented in the
		// hierarchy
		// If an employee is a worker and does not have a manager, do not add and return
		// false
		if (employee == null || (employee instanceof Worker && !employee.hasManager())) {
			return false;
		} else {
			this.employees.addAll(employee.getChainOfCommand());
...
//        return result;
		...
		}).map(employee -> (Manager) employee).collect(Collectors.toSet());
	}
...
//		for (Employee current : this.employees) {
//			if (current.hasManager() && current.getManager().equals(manager)) {
//				result.add(current);
//			}
//		}
//		return result;
		return this.employees.stream()
				.filter(employee -> employee.hasManager() && employee.getManager().equals(manager))
...
//		for (Manager current : getAllManagers()) {
//			result.put(current, getDirectSubordinates(current));
//		}
//		return result;
		Map<Manager, Set<Employee>> finalResult = 
				this.employees
					.stream()
					.filter(employee -> !employee.hasManager())
					.collect(Collectors.toMap(
						emp -> emp.getManager(),
						emp -> Collections.singleton(emp),
						(setA, setB) -> {
							Set<Employee> result = new HashSet<>();
							result.addAll(setA);
							result.addAll(setB);
		...
		for (Manager manager : this.getAllManagers()) {
			if (!finalResult.containsKey(manager)) {
				finalResult.put(manager, new HashSet<>());
			}
		}
